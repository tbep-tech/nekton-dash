...
rctseltab <- reactive({
  ...
  out <- reactable(tabsel,
    columns = list(
      Reference = colDef(name = 'Station'), 
      bay_segment = colDef(name = 'Bay segment'), 
      Action = colDef(style = function(value){
        
        color <- case_when(
          value == 'On Alert' ~ '#FF000080', 
          value == 'Caution' ~ '#FFFF00', 
          value == 'Stay the Course' ~ '#00FF0080'
        )  
...
shangauge <- reactive({
  ...
  out <- gaugefun(fimplo, tbniscr, cols, 'Shannon', 'Shannon diversity', raw = rawsel2)
