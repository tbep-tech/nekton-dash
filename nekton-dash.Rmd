...
attmap <- reactive({
  ...
  mapview::mapview(attyr, fill = F, homebutton = F, popup = NULL, legend = F) %>% 
    .@map %>% 
...
output$map <- renderLeaflet({
  ...
  # map with custom legends
  mapview::mapview(fimyrs, fill = F, homebutton = F, popup = NULL, legend = F) %>% 
    .@map %>% 
    leaflet::clearMarkers() %>%
    leaflet::addCircleMarkers(
      data = fimyrs, 
...
<span style="color:#2DC938; text-shadow: 0 0 3px #333;">__Stay the Course__</span>: Continue planned projects. Report data via annual progress reports. 

<span style="color:#E9C318; text-shadow: 0 0 3px #333;">__Caution__</span>: Review monitoring data and identify additional research/monitoring needs. Begin/continue TAC and Management Board development of specific management recommendations.

<span style="color:#CC3231; text-shadow: 0 0 3px #333;">__On Alert__</span>: Finalize development and implement appropriate management actions to get nekton community back on track.
...
In the above example, the action category for Hillsborough Bay in 2001 was <span style="color:#E9C318; text-shadow: 0 0 3px #333;">Caution</span>.  
...
The above example shows results for site TBM2019091504, which was assigned an action category of <span style="color:#2DC938; text-shadow: 0 0 3px #333;">__Stay the Course__</span> based on a TBNI score of 64.  
