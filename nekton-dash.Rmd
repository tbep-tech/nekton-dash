...
<br>
```{r, fig.align='center', out.width='70%', fig.cap = 'A degraded nekton community with low diversity and low abundance of organisms.  Water quality and habitat conditions may also be degraded.  Sites with __low TBNI scores__ may have similar characteristics.'}
knitr::include_graphics('www/lotbni.png')
```
<br>

<br>
```{r, fig.align='center', out.width='70%', fig.cap = 'A healthy nekton community with high diversity and high abundance of organisms.  Water quality and habitat conditions support a healthy nekton community.  Sites with __high TBNI scores__ may have similar characteristics.'}
knitr::include_graphics('www/hitbni.png')
```
<br>

TBNI scores can be translated into actionable management categories with narrative interpretations.  For example, scores above the median TBNI for the development dataset indicate a healthy nekton community with a management category of "Stay the Course".  Scores between the 33rd percentile and median are in the "Caution" category and scores below the 33rd percentile are in the "On Alert" category. Each action category has a narrative description to provide guidance to managers:
...
The map on the right shows results for a given year, where the color for each bay segment shows the action category outcome based on the average of station results in each segment. The individual stations are also shown and color-coded by the action categories. Hovering the mouse over a station or bay segment will reveal additional information.  
...
The above example shows results for station TBM2019091504, which was assigned an action category of <span style="color:#33FF3B; text-shadow: 0 0 3px #333;">__Stay the Course__</span> based on a TBNI score of 64.  

Finally, the year slider on the top can be used to select which year is shown on the map. A reference box on the *MATRIX RESULTS* tab and a reference line on the *TIME SERIES PLOTS* shows which year is selected. Selected the play button will cycle through all years in the data, with the maps and plots updating with each year. 
...
This tab shows TBNI results for selected stations, including an assessment of individual metric results.  An interactive map shows all stations across all years.  Begin by selecting a station from the map.  The selected station will be shown as a larger, darker point:
...
Information about the selected station will appear on the right as several rows of "gauges".  Text will also indicate which station was selected, including the sample year and month (as a number from 1-12). 
...
There are three rows of gauges that provide information about the selected station: 
